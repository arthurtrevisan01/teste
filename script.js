...
function renderHome(container){
  ...
  for(const [key, plan] of Object.entries(WORKOUT_PLANS)){
    html += `<div onclick="router('workout_select','${key}')" class="card cursor-pointer active:scale-95 transition transform border border-gray-700 hover:border-blue-500">
  ...
  if(appState.history.length>0){
    html += `<div class="mt-8 pt-4 border-t border-gray-700">
      <h3 class="text-lg font-bold mb-2">Último Treino</h3>
      <div onclick="alert('Repetir último treino (em breve)')" class="card bg-gray-800 opacity-75">
...
function renderWorkoutSelect(container, planKey){
  ...
  plan.days.forEach((day,index)=>{
    html += `<div onclick="startWorkout('${planKey}',${index})" class="card cursor-pointer active:scale-95 flex justify-between items-center">
      <div>
...
function startWorkout(planKey,dayIndex){
  ...
  appState.activeWorkout = {
    planName: plan.name,
    dayName: day.name,
    exercises: day.exercises.map(ex=>({name:ex, sets:[{weight:'',reps:'',rpe:''}]})),
    startTime: Date.now()
  };
  ...
  workout.exercises.forEach((ex,exIndex)=>{
    html += `<div class="card border border-gray-700">
      ...
      <div class="grid grid-cols-4 gap-2 text-xs text-gray-500 mb-1 text-center">
        <span>KG</span><span>REPS</span><span>RPE</span><span></span>
      </div>
...
function renderSetRow(exIndex,setIndex,setVal){
  return `<div class="grid grid-cols-4 gap-2 mb-2 items-center">
...
window.addSet = (exIndex)=>{
  const prev = appState.activeWorkout.exercises[exIndex].sets.slice(-1)[0];
  appState.activeWorkout.exercises[exIndex].sets.push({...prev});
  router('active_session');
};
...
window.finishWorkout = ()=>{
  ...
  clearInterval(window.workoutTimer); window.workoutTimer=null;
  const workout = appState.activeWorkout;
...
function analyzeWorkout(workout){
  let totalSets=0,hardSets=0,failureSets=0,totalVolume=0,missedData=0;
  workout.exercises.forEach(ex=>ex.sets.forEach(set=>{
    if(!set.weight||!set.reps){missedData++; return;}
    const w=parseFloat(set.weight),r=parseFloat(set.reps),rpe=parseFloat(set.rpe)||0;
    totalSets++; totalVolume+=w*r;
    if(rpe>=7) hardSets++;
    if(rpe>=9) failureSets++;
  }));
  let score=0,feedback=[],grade='C';
  ...
  if(intensityRatio>=0.8){score+=40; feedback.push("🔥 Intensidade Excelente!");}
  else if(intensityRatio>=0.5){score+=20; feedback.push("⚠️ Intensidade Média.");}
  else feedback.push("💩 Intensidade Baixa. Aumente a carga.");
  if(totalSets>=10 && totalSets<=25){score+=30; feedback.push("✅ Volume dentro do ideal.");}
  else if(totalSets<10) feedback.push("📉 Volume baixo.");
  else feedback.push("🛑 Volume excessivo.");
  if(failureSets>0){score+=30; feedback.push("💪 Você atingiu falha técnica em algumas séries.");}
...
function renderHistory(container){
  if(appState.history.length===0){
    container.innerHTML=`<div class="fade-in text-center mt-20 text-gray-500"><p>Nenhum treino realizado ainda.</p><p>Vá puxar ferro!</p></div>`; return;
  }
...
function renderSettings(container){
  ...
  container.innerHTML=`<div class="fade-in pb-20">
    ...
    <div class="mt-8">
      <button onclick="if(confirm('Apagar tudo?')){localStorage.clear();location.reload();}" class="w-full bg-red-600 hover:bg-red-700 text-white py-3 rounded-lg font-bold">Resetar Histórico</button>
    </div>
  </div>`;
}
